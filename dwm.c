...
void drawbar(Monitor *m)
{
    ...
    if (m == selmon || 1)
    { /* status is only drawn on all monitors */
...
void incnmaster(const Arg *arg)
{
    ...
    for (Client *c = selmon->clients; c; c = c->next)
        if (ISVISIBLE(c))
            n++;
    selmon->nmaster = selmon->pertag->nmasters[selmon->pertag->curtag] = MIN(MAX(selmon->nmaster + arg->i, 0), n ? n : 1);
    arrange(selmon);
}
...
void altTabEnd()
{
    ...
    {
        if (selmon->altTabN != 0)
        { /* if user picked original client do nothing */
            ...
            else
            {
                while (grabbed)
                {
                    ...
                    if (event.type == KeyPress || event.type == KeyRelease)
                    {
                        ...
                        { /* if super key is released break cycle */
                            break;
                        }
                        else if (event.type == KeyPress)
                        {
                            if (event.xkey.keycode == tabCycleKey)
                            { /* if XK_s is pressed move to the next window */
                                altTab();
                            }
                        }
                    }
